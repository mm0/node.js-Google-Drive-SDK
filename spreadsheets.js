...
Spreadsheets.rows = function(opts, cb) {
	...
	getFeed(["cells", opts.key,1], opts.auth, query, function(err, data) {
		if(err) {
			return cb(err);
		}
...
var Worksheet = function(spreadsheet, data) {
	...
	this.id = data.id[0].substring(data.id.lastIndexOf("/") + 1);
this.id =this.id.replace('https://spreadsheets.google.com/feeds/worksheets/','');
